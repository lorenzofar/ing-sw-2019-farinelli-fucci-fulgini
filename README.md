...
| :------------------ | :------------- | :------------- | :-------------------------------------- |
| Lorenzo Farinelli   | @lorenzofar    | 866236         | :computer: Client, Model                |
| Tiziano Fucci       | @tizianofucci  | 873622         | :satellite: Updates, Model              |
| Alessandro Fulgini  | @fuljo         | 866390         | :video_game: Controller, Network, Model |
...
clients.\
Before starting the server you must set the parameter
`adrenaline.server.hostname` in the `adrenaline.properties` file to the hostname
or IP address of the computer running the server. This address must be
resolvable and reachable by the clients.
...
Alternatively, you can start the server by double-clicking the JAR,
but it will run in headless mode and you won't see any console output.

You can also configure other parameters, for more details see the
[dedicated section](#configuration-parameters).
...
lobby until there are enough waiting players to start the match (3 by default).
The default waiting time is 30 seconds.
...
## Client
Before starting the client, if you want to use RMI connection, you must set the
parameter `adrenaline.client.hostname` in the `adrenaline.properties` file to
the hostname or IP address of the computer running the client. This address must
be resolvable and reachable by the server.\
If you want you can provide this parameter on command-line or with another
configuration file, see [this section](#configuration-parameters) for details.
...
The parameters can be loaded from a custom file by specifying the
...
parameters, so check the their correctness before running the program.
